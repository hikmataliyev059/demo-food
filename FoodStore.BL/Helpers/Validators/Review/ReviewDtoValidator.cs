...
public class ReviewDtoValidator : AbstractValidator<ReviewDto>
{
    public ReviewDtoValidator()
    {
        ...
        RuleFor(x => x.Comment).MaximumLength(1000).WithMessage(ValidationMessages.MaxLength);

        RuleFor(x => x.ProductId).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
