...
public class RegisterDtoValidator : AbstractValidator<RegisterDto>
{
    ...
    {
        RuleFor(x => x.Username).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
            .MinimumLength(3).WithMessage(ValidationMessages.MinLength)
            .MaximumLength(30).WithMessage(ValidationMessages.MaxLength);

        RuleFor(x => x.FirstName).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
            .MinimumLength(3).WithMessage(ValidationMessages.MinLength)
            .MaximumLength(30).WithMessage(ValidationMessages.MaxLength);

        RuleFor(x => x.LastName).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
            .MinimumLength(3).WithMessage(ValidationMessages.MinLength)
            .MaximumLength(30).WithMessage(ValidationMessages.MaxLength);
        ...
        RuleFor(x => x.Password).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
            .MinimumLength(4).WithMessage(ValidationMessages.MinLength)
            .Matches("^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$")
            .WithMessage(ValidationMessages.PasswordCriteria);

        RuleFor(x => x.ConfirmPassword).NotEmpty().NotNull().WithMessage(ValidationMessages.Required)
            .MinimumLength(4).WithMessage(ValidationMessages.MinLength)
